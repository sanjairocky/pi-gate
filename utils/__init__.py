...
def get_next_env(config, curr_env):
    higher_env = None
    for i, e in enumerate(config['stages_order']):
        if e == curr_env and i+1 < len(config['stages_order']):
            higher_env = config['stages_order'][i+1]
    return higher_env


def convert_filter_scopes(
    json_metadata: Dict[Any, Any], filters: list
...
__init__ =[
    'convert_filter_scopes', 'copy_filter_scopes', 'get_next_env'
]